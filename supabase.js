...
const DB_FILES = ['users.json','products.json','transactions.json','testimonials.json','notifications.json','settings.json','keyspool.json','vouchers.json','admin-lock.json','accounts.json','login-attempts.json'];

// File yang defaultnya object {} bukan array [] saat cache masih kosong
const OBJECT_FILES = new Set(['settings.json', 'admin-lock.json', 'login-attempts.json']);
...
// ══════════════════════════════════════════════════════════════════
// atomicUpdate — compare-and-swap read-modify-write LINTAS-INSTANCE.
//
// FIX BUG KEAMANAN (audit 3 Sep 2026): operasi yang menyentuh SALDO WALLET
// (potong saldo saat /wallet/buy, kreditkan saldo saat top up berhasil)
// sebelumnya cuma dilindungi lock in-memory per-proses Node (`walletLocks`
// Set). Itu cukup untuk 1 server biasa, TAPI Vercel menjalankan banyak
// instance serverless yang tidak berbagi memori sama sekali — dua request
// hampir bersamaan yang kebetulan jatuh ke instance BERBEDA bisa saja
// lolos dari lock in-memory itu, baca saldo lama yang sama, lalu
// keduanya menulis hasil kurang/tambah dari basis yang sama (lost
// update / double-spend saldo).
//
// atomicUpdate menutup celah itu dengan compare-and-swap di level baris
// Postgres: ambil value SEKARANG dari Supabase, jalankan mutateFn di
// atasnya, lalu tulis balik HANYA JIKA value di DB masih persis sama
// dengan yang barusan dibaca (`.eq('value', current)` di WHERE clause).
// Kalau instance lain sempat menulis duluan di antara baca dan tulis
// kita, update ini tidak akan match baris manapun (0 rows affected) —
// artinya kita tahu terjadi konflik, lalu otomatis retry dari awal
// (baca ulang state TERBARU, jalankan mutateFn lagi, coba tulis lagi).
// Ini bikin operasi read-modify-write jadi atomik secara efektif tanpa
// perlu ubah skema tabel (masih tabel key-value JSON blob yang sama).
//
// mutateFn(currentData) HARUS pure function (tidak side-effect di luar
// return value-nya) karena bisa dipanggil ulang beberapa kali kalau ada
// konflik. Return salah satu:
//   { ok: false, reason: '...' }              → batal, tidak menulis apa-apa
//   { ok: true, data: newData, result: {...} } → coba tulis newData
//
// Kalau Supabase tidak terkonfigurasi (dev lokal tanpa env), fallback ke
// baca-ubah-tulis biasa (non-atomic) — sama seperti perilaku lama, cukup
// aman untuk 1 proses lokal karena walletLocks in-memory masih berlaku di
// situ.
// ══════════════════════════════════════════════════════════════════
const atomicUpdate = async (filename, mutateFn, { maxRetries = 5 } = {}) => {
  const client = getClient();
  if (!client) {
    // Fallback non-atomic (dev lokal / Supabase belum dikonfigurasi)
    const current = readDB(filename);
    const outcome = mutateFn(current);
    if (!outcome.ok) return outcome;
    await writeDB(filename, outcome.data);
    return outcome;
  }
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    let current;
    try {
      const { data, error } = await client.from('keyvalue_store').select('value').eq('key', filename).single();
      if (error) throw new Error(error.message);
      current = data?.value !== undefined ? data.value : (OBJECT_FILES.has(filename) ? {} : []);
    } catch (e) {
      // Gagal baca dari Supabase — fallback ke cache lokal, tetap coba jalan
      current = readDB(filename);
    }

    const outcome = mutateFn(current);
    if (!outcome.ok) return outcome; // mutateFn sendiri yang menolak (mis. saldo tidak cukup) — bukan soal race condition

    try {
      const { data: updated, error: updErr } = await client
        .from('keyvalue_store')
        .update({ value: outcome.data })
        .eq('key', filename)
        .eq('value', current)
        .select('key');
      if (updErr) throw new Error(updErr.message);
      if (updated && updated.length > 0) {
        // Sukses — tidak ada instance lain yang menulis di antara baca & tulis kita
        dbCache[filename] = outcome.data;
        cacheTimestamp[filename] = Date.now();
        writeLocalBackup(filename, outcome.data);
        return outcome;
      }
      // updated.length === 0 → konflik (ada yang nulis duluan), retry dari baca ulang
    } catch (e) {
      console.error(`[supabase] atomicUpdate ${filename} write error (attempt ${attempt + 1}):`, e.message);
      // lanjut retry — mungkin error transient
    }
  }
  return { ok: false, reason: 'Gagal memproses karena server sedang sibuk, coba lagi sebentar.' };
};
...
module.exports = { readDB, writeDB, initializeDB, getDbStatus, uploadImage, refreshFromDB, readFresh, readSmart, getClient, atomicUpdate };
